<?php
		...
		$sql_btm="select * from tb_menu WHERE public_menu=1 order by menu_sequence ASC";
		...
		while($rs_btm=mysql_fetch_array($result_btm)) {
			...
			$sql14="select id_menu from tb_menu WHERE sub_of='$s1_id_menu_btm' AND name_menu='Overview' AND public_menu=1;";
			...
			if ($s1_level_btm==1){
				...
				echo '<ul>';
				$sql5_btm="select * from tb_menu where public_menu=1 order by menu_sequence ASC" ;
				...
				while($rs5_btm=mysql_fetch_array($result5_btm)) {
					...
					if($s2_sub_of_btm==$s1_id_menu_btm && $s2_name_menu_btm!="Overview"){
						////////////////////////////////////////////////// Sub LV1 overview
