<?php
	...
	while ($level_L1!=1){
		...
		$sql51="SELECT *  FROM `tb_menu_th` WHERE sub_of='$keep_select1' AND name_menu='ภาพรวม'";
		$result51=mysql_db_query($dbname,$sql51);
		$rs51=mysql_fetch_array($result51);
		$id_menu51=$rs51[id_menu];
		$sql51="SELECT *  FROM `tb_menu_th` WHERE id_menu='$keep_select1' AND public_menu=1";
		$result51=mysql_db_query($dbname,$sql51);
		$rs51=mysql_fetch_array($result51);
		$name_menu51=$rs51[name_menu];
		
		$sql52="SELECT *  FROM `tb_menu_th` WHERE id_menu='$keep_select2' AND public_menu=1";
		$result52=mysql_db_query($dbname,$sql52);
		$rs52=mysql_fetch_array($result52);
		$name_menu52=$rs52[name_menu];
		$id_menu52=$rs52[id_menu];
		
		$sql53="SELECT *  FROM `tb_menu_th` WHERE id_menu='$keep_select3' AND public_menu=1";
		$result53=mysql_db_query($dbname,$sql53);
		$rs53=mysql_fetch_array($result53);
		$name_menu53=$rs53[name_menu];
		$id_menu53=$rs53[id_menu];
		
		$sql54="SELECT *  FROM `tb_menu_th` WHERE id_menu='$keep_select4' AND public_menu=1";
