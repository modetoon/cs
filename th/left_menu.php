<?php
	...
	$sql_L1="SELECT *  FROM `tb_menu_th` WHERE id_menu='$call_arti' AND public_menu=1";
	...
	while ($level_L1!=1){
		$sql_L2="SELECT *  FROM `tb_menu_th` WHERE id_menu='$sub_of_L1' AND public_menu=1";
		...
		if ($level_L1==3){
			...
			$keep_select2=$call_left_menu;
				...
				while($rs5=mysql_fetch_array($result51)) {
					...
					if($s2_sub_of==$call_left_menu && $s2_name_menu==ภาพรวม){
						////////////////////////////////////////////////// Sub LV1 overview
						if($s2_id_menu==$keep_select2){
							echo '<li class="selected"><a href="subhomepage.php?ID='.$s2_id_menu.'" class="selected">ภาพรวม</a></li>';
						}else{
							echo '<li><a href="subhomepage.php?ID='.$s2_id_menu.'">ภาพรวม</a></li>';
						}
						$optionlv2++;
					}
				}
							...
							while($rs6=mysql_fetch_array($result61)) {
								...
								////////////////////////////////////////////////// Sub LV2 overview
									if($s3_id_menu==$keep_select3){
										echo '<li class="selected"><a href="subhomepage.php?ID='.$s3_id_menu.'" class="selected">-- ภาพรวม</a></li>';
									}else{
										echo '<li><a href="subhomepage.php?ID='.$s3_id_menu.'">-- ภาพรวม</a></li>';
									}
								}
							}
							while($rs6=mysql_fetch_array($result62)) {
								...
								////////////////////////////////////////////////// Sub LV2
									...
									while($rs7=mysql_fetch_array($result71)) {
										...
										if($s4_id_menu==$s3_id_menu && $s4_name_menu==ภาพรวม){
										////////////////////////////////////////////////// Sub LV3 overview
											if($s4_id_menu==$keep_select4){
												echo '<li class="selected"><a href="subhomepage.php?ID='.$s4_id_menu.'" class="selected">ภาพรวม</a></li>';
											}else{
												echo '<li><a href="subhomepage.php?ID='.$s4_id_menu.'">ภาพรวม</a></li>';
											}
										}
									}
