<?php
	...
	while ($level_L1!=1){
		...
		$call_active_menu=$rs_L2[id_menu];
		$level_L1=$rs_L2[level];
	}
	$sql51="SELECT *  FROM `tb_menu` WHERE sub_of='$call_active_menu' AND name_menu='Overview'";
	$result51=mysql_db_query($dbname,$sql51);
	$rs51=mysql_fetch_array($result51);
	$call_active_menu=$rs51[id_menu];

	$sql="select * from tb_menu WHERE public_menu=1 order by menu_sequence ASC";
	...
	while($rs=mysql_fetch_array($result)) {
			...
			$sql14="select id_menu from tb_menu WHERE sub_of='$s1_id_menu' AND name_menu='Overview' AND public_menu=1;";
			...
			if ($s1_level==1){
			///////////////////////////////////// Main Menu
			if ($call_active_menu==$s14_id_menu){
				echo '<li class="n2 selected"><a href="subhomepage.php?ID='.$s14_id_menu.'" class="haschild selected" >'.$s1_name_menu.'</a>';
			}else{
			...
			echo '<div class="lnk"><a href="subhomepage.php?ID='.$s14_id_menu.'">Overview</a></div>';
			echo '</li>';
			...
			echo '<ul>';
				while($rs5=mysql_fetch_array($result51)) {
					...
					if($s2_sub_of==$s1_id_menu && $s2_name_menu==Overview){
						////////////////////////////////////////////////// Sub LV1 overview
						echo '<li class="notmobile"><a href="subhomepage.php?ID='.$s2_id_menu.'">Overview</a></li>';
					}
				}
				while($rs5=mysql_fetch_array($result52)) {
					...
					if($s2_sub_of==$s1_id_menu && $s2_name_menu!=Overview){
						////////////////////////////////////////////////// Sub LV1
						if($s2_have_sub==1){
						echo '<li class="haschild"><a href="contentpage.php?ID='.$s2_id_menu.'" class="haschild">'.$s2_name_menu.'</a><ul>';
						...
						while($rs6=mysql_fetch_array($result61)) {
							...
							if($s3_sub_of==$s2_id_menu && $s3_name_menu==Overview){
								////////////////////////////////////////////////// Sub LV2 overview
								echo '<li class="notmobile"><a href="subhomepage.php?ID='.$s3_id_menu.'">Overview</a></li>';
							}
						}
						while($rs6=mysql_fetch_array($result62)) {
							...
							if($s3_sub_of==$s2_id_menu && $s3_name_menu!=Overview){
								////////////////////////////////////////////////// Sub LV2
								if($s3_have_sub==1){
									echo '<li class="haschild"><a href="contentpage.php?ID='.$s3_id_menu.'" class="haschild">'.$s3_name_menu.'</a><ul>';
								...
								while($rs7=mysql_fetch_array($result71)) {
									...
									if($s4_sub_of==$s3_id_menu && $s4_name_menu==Overview){
										////////////////////////////////////////////////// Sub LV3 overview
										echo '<li class="notmobile"><a href="subhomepage.php?ID='.$s4_id_menu.'">Overview</a></li>';
									}
								}
	...
	///////////////////////////Job & Career/////////////////
    	...
    	echo '<li class="n5"><a href="press_content.php" class="haschild ">Press</a>
          <ul class="newsub">
            <li class="megaTsrBx">
              <h2 class="thdln">Press</h2>
              <a href="press_content.php"><img class="lazy" width="170" height="100" src="../img/sys/ph.png" alt="" data-original="'.$s27_menu_image.'"/></a>
			 
              <p>'.$s27_menu_intro.'</p>
              <div class="lnk"><a href="press_content.php">Overview</a></div>
            </li>
            
            <li class="newlevel2">
              <ul>
              <li class="notmobile"><a href="press_content.php">Press</a></li>
		...
		///////////////////////////Job & Career/////////////////
			$sql28="select * from tb_menu WHERE name_menu='Job & Career'";
        ...
        <li class="n6"><a href="jobs_content.php" class="haschild ">Job & Career</a>
          <ul class="newsub">
            <li class="megaTsrBx">
              <h2 class="thdln">Job & Career</h2>
              <a href="jobs_content.php"><img class="lazy" width="170" height="100" src="../img/sys/ph.png" alt="" data-original="'.$s28_menu_image.'"/></a>
              <p>'.$s28_menu_intro.'</p>
              <div class="lnk"><a href="jobs_content.php">Overview</a></div>
            </li>
            <li class="newlevel2">
              <ul>
			  <li class="notmobile"><a href="jobs_content.php">Job & Career</a></li>
              </ul>
            </li>
          </ul>
        </li>';
