...
<?php include"header_name.php"; ?>         
	...
	$sql="select * from tb_article where `id_article` = $id_edit";
	...
	if($hidAction == "Add"){               //เช็คค่า hidAction ที่ส่งมาถ้าเท่ากับ Add ให้โปรแกรทำงานต่อ
				$sql11="UPDATE tb_article_backup set menu_article='$xhide', topic_article='$topic_article', content_slider='$content_slider', content_article='$content_article', show_rmenu='$totalmenu', modify_article='$ndate' where id_article='$id_edit' ";
				$result11=mysql_db_query($dbname,$sql11) or die(mysql_error());
				
				$sql12="UPDATE tb_article_th_backup set menu_article='$xhide', topic_article='$topic_article_th', content_slider='$content_slider_th', content_article='$content_article_th', show_rmenu='$totalmenu', modify_article='$ndate' where id_article='$id_edit' ";
				$result12=mysql_db_query($dbname,$sql12) or die(mysql_error());
		if ($approve==1){	
			if ($count==1){
				...
				$sql="INSERT INTO `tb_article` (`id_article` ,`lang_article` ,`menu_article` ,`type_article` ,`topic_article` , `content_slider` , `content_article` ,`show_rmenu` ,`create_article` ,`modify_article`)VALUES ('$id_edit', '$lang_article', '$xhide', '$type_article', '$topic_article', '$content_slider', '$content_article', '$totalmenu', '$ndate', '$ndate');";
				$result=mysql_db_query($dbname,$sql) or die(mysql_error());
				
				$sql2="INSERT INTO `tb_article_th` (`id_article` ,`lang_article` ,`menu_article` ,`type_article` ,`topic_article` , `content_slider` , `content_article` ,`show_rmenu` ,`create_article` ,`modify_article`)VALUES ('$id_edit', '$lang_article', '$xhide', '$type_article', '$topic_article_th', '$content_slider_th', '$content_article_th', '$totalmenu', '$ndate', '$ndate');";
				$result2=mysql_db_query($dbname,$sql2) or die(mysql_error());
			}
		}
				if ($result)  {
