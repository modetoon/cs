...
$(document).ready(function(){
	...
	bootbox.prompt($('#insert_folder_name').val(),$('#cancel').val(),$('#ok').val(), function(name) {
		...
		$.ajax({
			  type: "POST",
			  url: "execute2.php?action=create_folder",
		...
		}, 
		{
			...
			"callback": function() {
                ...
                if (newFileName !== null) 
                {
                	...
                	$.ajax({
					type: "POST",
					url: "execute2.php?action=create_file",
		...
		[
			...
			}, 
			{
				...
				"callback": function() {
                	...
                	$.ajax({
					type: "POST",
					url: "execute2.php?action=save_text_file",
...
function paste_to_this_dir(dnd) {
	bootbox.confirm($('#lang_paste_confirm').val(),$('#cancel').val(),$('#ok').val(), function(result) {
		if (result == true){
			...
			$.ajax({
			type: "POST",
			url: "execute2.php?action=paste_clipboard",
			data: {path: folder_path, path_thumb: folder_path_thumb}
			}).done(function( msg ) {
				if (msg!=""){
					bootbox.alert(msg);
...
function execute_action(action,file1,file2,name,container,function_name){
	...
	$.ajax({
	    type: "POST",
	    url: "execute2.php?action="+action,
