...
170, 100);
	...
	$sql21="INSERT INTO `tb_press_backup` (`id_press` ,`press_type` ,`topline` ,`headerline` ,`news_date` ,`show_type` ,`press_pic` ,`press_content` ,`view_count` ,`upload_date` ,`mod_date` ,`public_press`)VALUES ('$id_edit', '$press_type', '$topline', '$headerline', '$press_date', '$show_type', '$upload_path', '$content_article', '0', '$ndate', '$ndate', '0');";  
	$result21=mysql_db_query($dbname,$sql21) or die(mysql_error());
	$sql22="INSERT INTO `tb_press_th_backup` (`id_press` ,`press_type` ,`topline` ,`headerline` ,`news_date` ,`show_type` ,`press_pic` ,`press_content` ,`view_count` ,`upload_date` ,`mod_date` ,`public_press`)VALUES ('$id_edit', '$press_type', '$topline_th', '$headerline_th', '$press_date', '$show_type', '$upload_path', '$content_article_th', '0', '$ndate', '$ndate', '0');";   
	$result22=mysql_db_query($dbname,$sql22) or die(mysql_error());
		if ($result21){
			echo"<h3>You have added new Press successfully.</h3><br />";
			echo"<p><span class='txtnormal'>Click <a href='user_press.php'>here</a> for back to Press main page.</span></p>";
			}else{			
			echo"<p><span style='color:#F00'>You cannot add a new Press, Please kindly try again or contact your IT team.</span></p><br />";
			echo"<p><span class='txtnormal'>Click <a href='user_press.php'>here</a> for back to Press main page.</span></p>";
			}
